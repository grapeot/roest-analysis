...
### 2026-03-27 — 案例研究补足诊断层，并合并项目内说明入口

这轮改动集中在两个点。

第一，README 的案例研究不再只停留在技术指标展示，而是补上了诊断链条。第一轮案例现在会把技术特征、杯测表现和可执行的 roast 调整建议连起来：development 偏短不仅是一个数字现象，也对应 espresso 的尖酸、奶饮穿透力不足和 body 偏空。第三轮案例则明确强调它的价值主要在技术诊断：面对 split-cluster crack，不去假装有一个绝对正确的单点答案，而是把歧义本身转化为下一锅的调整方向。

第二，项目内说明文件去重。`AGENTS.md` 原先和 `CLAUDE.md` 存在明显重叠，现在改成直接指向 `CLAUDE.md` 的 symlink，让项目只有一个权威说明源，避免后续继续漂移。

随后又补了一个校正：可视化里的 ROR 单位从每秒改成了更符合烘焙习惯的每分钟，并重新生成了公开示例图。README 第二个案例的建议也同步收窄到热量管理和控制动作本身，不再把 crack 是否分簇当成直接可控的目标。

又做了一层展示修正：ROR 图不再强调回温前那段负值。对可视化来说，真正有诊断价值的是从 0 开始进入正向升温之后的部分，因此前段 warmup 的负 ROR 被压掉，ROR 面板的纵轴也固定从 0 起。

最后又把 README 里的案例建议写得更具体了一层，尽量贴近实际 roast memo 的写法：第一轮明确落到“一爆后发展不够，因此下一锅要拉长 development、让一爆前后能量衔接更平顺”；第三轮明确落到“重点看 crack 前 30–60 秒的 heat / fan 和进入一爆时的热动量”，而不是停留在泛泛的技术描述。

再往前推了一步之后，案例研究本身也改成了更明确的 memo 结构：不再只是连续段落，而是显式拆成 `What / Diagnosis / Risks / Next`。这样读者一眼就能看到这套系统不是只会展示图和指标，而是真的在做“证据 → 判断 → 风险 → 下一锅动作”的 roast diagnostics。
...
- 仓库已创建并推送到 public GitHub，后续变更直接在该远端继续迭代
